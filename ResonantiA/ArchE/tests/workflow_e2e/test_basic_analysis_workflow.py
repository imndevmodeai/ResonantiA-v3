...
def test_basic_analysis_workflow_e2e(e2e_engine: WorkflowEngine, mocker):
    ...
    # or functions that would cause external calls / config checks.
    ...
    # Mock LLM provider functions and key validation to prevent real API key checks.
    mock_is_valid_key = mocker.patch('ResonantiA.ArchE.tools.is_valid_api_key', return_value=True)
    mock_llm_model_getter = mocker.patch('ResonantiA.ArchE.llm_providers.get_model_for_provider', return_value="mock_model_name")
