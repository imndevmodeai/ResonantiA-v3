...
class SPRManager:
    ...
    def conceptual_write_spr(self, core_concept_term: str, definition: str, relationships: dict, blueprint: str, category: str = "General", **metadata) -> Optional[str]:
        ...
        spr_def = {
            ...
            "example_usage": metadata.get("ExampleUsage", ""), # MODIFIED
            "metadata": { # MODIFIED
    ...
    def remove_spr(self, spr_id: str) -> bool:
        """
        Removes an SPR definition from the manager and saves to file.
        Requires 'spr_id'.

        Args:
            spr_id (str): The ID of the SPR to remove.

        Returns:
            bool: True if the SPR was successfully removed, False otherwise.
        """
        if not isinstance(spr_id, str) or not spr_id:
            logger.error("Invalid or empty spr_id provided to remove_spr.")
            return False
        if spr_id not in self.sprs:
            logger.warning(f"SPR with ID '{spr_id}' not found in manager. Remove failed.")
            return False
        # Remove the SPR from the in-memory dictionary
        removed_spr = self.sprs.pop(spr_id)
        logger.info(f"Removed SPR: '{spr_id}' (Term: '{removed_spr.get('term')}')")
        # Persist changes to the file
        self.save_sprs()
        return True
