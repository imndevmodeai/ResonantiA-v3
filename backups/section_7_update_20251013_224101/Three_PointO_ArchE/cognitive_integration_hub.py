...
class CognitiveIntegrationHub:
    ...
    def route_query(self, query: str, model: str = None) -> CognitiveResponse:
        ...
        if crcs_response.confidence.collapse() >= effective_threshold:
            ...
            try:
                # Pass the model parameter down to the RISE orchestrator
                final_response = self.rise.process_query(query, {"crcs_context": crcs_response.metadata}, model=model)
                self.thought_trail.add_entry(final_response.to_dict()) # Log RISE response
            except Exception as e:
