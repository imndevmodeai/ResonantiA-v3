...
def run_puppeteer_search(inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    js_script_path = os.path.join("ResonantiA", "browser_automation", "search.js")
    ...
    from .code_executor import execute_code # Local import to avoid circular dependency with action_registry

    execute_code_result = execute_code({
        "language": "bash", # Run as bash command to execute node directly
        "code": js_code_to_run,
        "timeout": 90 # Increased timeout for browser ops
    })

    # Process the result from the `execute_code` action
    stdout = execute_code_result.get("stdout", "")
    stderr = execute_code_result.get("stderr", "")
    exit_code = execute_code_result.get("exit_code", -1)
    exec_error = execute_code_result.get("error")
