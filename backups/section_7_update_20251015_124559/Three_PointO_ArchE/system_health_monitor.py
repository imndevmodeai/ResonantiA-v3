...
class SystemHealthMonitor:
    ...
    def __init__(
        ...
        self.monitoring_start_time = now()
    ...
    def collect_metrics(self) -> Dict[str, HealthMetric]:
        ...
        if self.learning_loop and hasattr(self.learning_loop, 'get_metrics'):
            try:
                ...
                uptime_hours = (now() - self.monitoring_start_time).total_seconds() / 3600
        ...
        # System uptime
        uptime_seconds = (now() - self.monitoring_start_time).total_seconds()
            ...
            "active_alerts": [
                {
                    ...
                    "age_seconds": (now() - datetime.fromisoformat(alert.timestamp)).total_seconds()
                }
                for alert in latest_snapshot.active_alerts
            ],
            "metrics_summary": latest_snapshot.metrics_summary,
            "system_info": {
                "monitoring_uptime_hours": (now() - self.monitoring_start_time).total_seconds() / 3600,
