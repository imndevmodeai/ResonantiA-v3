...
# --- Default Agent and Model Implementations ---
# (Provide basic examples that can be overridden or extended)
class BasicGridAgent(Agent):
    """
    A simple agent for grid-based models. It can be active or inactive.
    """
    def __init__(self, unique_id, model, state=1, custom_params=None):
        """
        Args:
            unique_id: The agent's unique identifier.
            model: The model instance the agent belongs to.
            state: The initial state of the agent (1 for active, 0 for inactive).
            custom_params: A dictionary of custom parameters for the agent.
        """
...
class BasicGridModel(Model if MESA_AVAILABLE else object):
    ...
    def __init__(self, width=10, height=10, density=0.5, activation_threshold=2, agent_class: Type[Agent] = BasicGridAgent, scheduler_type='random', torus=True, seed=None, **model_params):
        ...
        # Mesa init - CORRECTED ORDER
        super().__init__(seed=seed) # 1. Call parent constructor FIRST to initialize schedule
        ...
        self.custom_agent_params = model_params.pop('agent_params', {})
        self.model_params = model_params

        # 3. Setup grid and scheduler type check
        ...
        # 5. Determine the number of agents to create from density
        self.num_agents = int(self.width * self.height * self.density)

        # 6. Create agents and place them
        self._create_agents_mesa() 

        # 7. Finalize setup
        self.running = True
        self.datacollector.collect(self)
        logger.info(f"Initialized Mesa BasicGridModel (Run ID: {self.run_id}) with {self.schedule.get_agent_count()} agents using '{self._scheduler_type}' activation style.")

    def _create_agents_mesa(self):
        """Internal helper to create agents using Mesa."""
        # Create agents
        for i in range(self.num_agents):
            unique_id = i
            initial_state = 1 if self.random.random() < self.density else 0
            agent = self.agent_class(unique_id, self, state=initial_state, custom_params=self.custom_agent_params)
            self.schedule.add(agent)
