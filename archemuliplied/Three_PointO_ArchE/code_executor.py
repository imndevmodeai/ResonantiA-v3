...
def execute_code(
    inputs: Dict[str, Any],
    context_for_action: Optional['ActionContext'] = None
) -> Dict[str, Any]:
    """
    ...
    Args:
        inputs (Dict): A dictionary containing necessary parameters.
        context_for_action (ActionContext, optional): The context of the action being executed.
    """
    ...
    timeout = inputs.get("timeout", TIMEOUT_SECONDS)
    ...
    if code is None and code_path is None:
        return {
            ...
            "reflection": create_reflection(
                ...
                inputs=inputs, execution_time=time.time() - start_time
            )
        }
        
    # Determine the execution method based on configuration and availability
    final_sandbox_method = sandbox_method_resolved
    if final_sandbox_method == 'docker' and not DOCKER_AVAILABLE:
        logger.warning("Docker sandbox was configured, but Docker is not available. Falling back to 'subprocess'.")
        final_sandbox_method = 'subprocess'

    exec_result: Dict[str, Any] = {}
    try:
        # Prepare code content: if path is given, use it as code, otherwise use code string
        code_content_or_path = code_path if code_path else code
        if not code_content_or_path:
             raise ValueError("Code content or path is empty.")

        # Dispatch to the chosen execution method
        logger.info(f"Dispatching code execution to sandboxing method: '{final_sandbox_method}'")
        if final_sandbox_method == 'docker':
            exec_result = _execute_with_docker(language, code_content_or_path, input_data, environment)
        elif final_sandbox_method == 'subprocess':
            # Pass the full runtime context to subprocess for injection
            full_context = context_for_action.runtime_context if context_for_action else {}
            exec_result = _execute_with_subprocess(language, code_content_or_path, input_data, environment, context=full_context)
        else: # 'none'
            logger.critical("Executing code with NO SANDBOX. This is a major security risk and should only be used in trusted environments.")
            full_context = context_for_action.runtime_context if context_for_action else {}
            exec_result = _execute_with_subprocess(language, code_content_or_path, input_data, environment, context=full_context)
            
        execution_time = time.time() - start_time
        
        # --- IAR Generation from Execution Result ---
        stdout = exec_result.get("stdout", "")
        stderr = exec_result.get("stderr", "")
        exit_code = exec_result.get("exit_code", -1)
        internal_error = exec_result.get("error")
        
        if internal_error: # Error in the execution logic itself
            return {
                "error": internal_error,
                "reflection": create_reflection(
                    action_name=action_name, status=ExecutionStatus.CRITICAL_FAILURE,
                    message=f"Code execution framework failed: {internal_error}",
                    inputs=inputs, execution_time=execution_time
                )
            }
        ...
        if stdout and stdout.strip().startswith('{') and stdout.strip().endswith('}'):
            try:
                ...
                pass # Not valid JSON, treat as raw string output

        outputs = {"output": parsed_output or stdout, "stderr": stderr, "exit_code": exit_code}
        ...
        if exit_code == 0:
            return {
                ...
                "reflection": create_reflection(
                    ...
                    message="Code executed successfully.", inputs=inputs, outputs=outputs,
                    confidence=1.0, execution_time=execution_time
                )
            }
        else:
            return {
                "result": outputs, # Still return output/stderr on failure
                "error": f"Code execution failed with exit code {exit_code}.",
                "reflection": create_reflection(
                    action_name=action_name, status=ExecutionStatus.FAILURE,
                    message=f"Code execution failed with return code {exit_code}.",
                    inputs=inputs, outputs=outputs, confidence=0.1,
                    potential_issues=[f"Return Code: {exit_code}", stderr],
        ...
        execution_time = time.time() - start_time
        error_msg = f"An unexpected error occurred in the execution dispatcher: {str(e)}"
        logger.error(f"Error in '{action_name}': {error_msg}", exc_info=True)
        return {
            "error": error_msg,
            "reflection": create_reflection(
                action_name=action_name, status=ExecutionStatus.CRITICAL_FAILURE,
                message=error_msg, inputs=inputs,
                potential_issues=[str(e)], execution_time=execution_time
            )
        }
