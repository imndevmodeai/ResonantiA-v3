...
async function searchDuckDuckGo(query) {
    ...
    try {
        ...
        // Wait for results
        await page.waitForSelector('article.react-results--mainline', { timeout: 15000 });
        
        // Extract results
        const results = await page.evaluate(() => {
            const items = [];
            const resultElements = document.querySelectorAll('article.react-results--mainline');
            
            resultElements.forEach((element, index) => {
                if (index >= 10) return; // Limit to 10 results
                
                const titleElement = element.querySelector('a[data-testid="result-title-a"]');
                const snippetElement = element.querySelector('span[data-testid="result-snippet"]');
