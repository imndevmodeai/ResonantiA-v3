...
class SimulatedLLMProvider(BaseLLMProvider):
    """LLM Provider implementation for simulating LLM calls for testing."""
    def _initialize_client(self) -> Optional[Any]:
        """No client needed for simulation."""
        logger.info("SimulatedLLMProvider initialized.")
        return "SimulatedClient"

    def generate(self, prompt: str, model: str, max_tokens: int = 500, temperature: float = 0.7, **kwargs) -> str:
        """Simulates a response based on keywords in the prompt."""
        prompt_lower = prompt.lower()
        if "intervention" in prompt_lower and "causal analysis" in prompt_lower:
            # Specific response for the showcase query
            return "Based on the significant lagged effect of the 'confounder' variable, we propose a targeted marketing campaign adjustment. The intervention will involve shifting promotional activities by two periods to counteract the cyclical confounding effect. This is intended to isolate the true impact of our primary treatment variable, leading to a more accurate assessment of marketing ROI and improved resource allocation."
        elif "summarize" in prompt_lower:
            return "This is a simulated summary of the provided text."
        else:
            return f"This is a simulated LLM response to the prompt: '{prompt[:80]}...'"

    def generate_chat(self, messages: List[Dict[str, str]], model: str, max_tokens: int = 500, temperature: float = 0.7, **kwargs) -> str:
        """Simulates a chat response by using the last user message."""
        last_user_message = "No user message found."
        for msg in reversed(messages):
            if msg.get("role") == "user":
                last_user_message = msg.get("content", "")
                break
        return self.generate(last_user_message, model, max_tokens, temperature, **kwargs)

# --- Provider Factory ---
# Maps provider names (lowercase) to their implementation classes.
PROVIDER_MAP: Dict[str, Type[BaseLLMProvider]] = {
    "simulated": SimulatedLLMProvider
}
if OPENAI_AVAILABLE:
