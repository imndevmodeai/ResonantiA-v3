...
AUTH_USERNAME = os.getenv('FACECHECK_AUTH_USER', 'admin')
AUTH_PASSWORD = os.getenv('FACECHECK_AUTH_PASS', 'changeme123')
...
# Enable/disable auth based on environment
ENABLE_AUTH = os.getenv('FACECHECK_ENABLE_AUTH', 'false').lower() == 'true'
...
# --- Routes ---
@app.route('/')
@auth.login_required if ENABLE_AUTH else lambda f: f
def index():
...
@app.route('/health')
def health():
    return jsonify({'status': 'ok', 'auth_enabled': ENABLE_AUTH}), 200
