...
class PlaybookOrchestrator:
    ...
    def execute_agent_action_plan(self, action_plan: Dict[str, Any], context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Execute the action plan generated by RISE Specialist Agent
        
        Args:
            action_plan: The agent's action plan with phases, capabilities, and sequences
            context: Optional execution context
            
        Returns:
            Execution results with all phases completed
        """
        if not action_plan:
            raise ValueError("Action plan is required")
            
        logger.info(f"Executing Agent Action Plan")
        
        # Display agent's rationale
        self._display_agent_rationale(action_plan)
        
        # Convert action plan to executable format
        executable_plan = self._convert_action_plan_to_playbook(action_plan)
        
        # Execute phases
        results = {
            'agent_action_plan': action_plan,
            'phases': [],
        ...
        try:
            for phase_idx, phase in enumerate(action_plan.get('action_sequence', [])):
                logger.info(f"Executing Phase {phase_idx + 1}: {phase['phase']}")
                
                # Check if actions can be parallelized
                if phase.get('parallel', False):
                    phase_result = self._execute_parallel_phase(phase, context, results)
                else:
                    phase_result = self._execute_sequential_phase(phase, context, results)
                
                results['phases'].append(phase_result)
                
                # Reassessment gate
                reassessment = self._needs_reassessment(phase_result, phase)
                if reassessment:
                    results['reassessments'].append(reassessment)
                    logger.warning(f"Reassessment triggered in phase {phase['phase']}")
                    
                    if reassessment.get('action') == 'regenerate_plan':
                        results['execution_status'] = 'AGENT_REPLANNING_REQUIRED'
                        results['reassessment'] = reassessment
                        return results
        except Exception as e:
            logger.error(f"Error executing agent action plan: {e}")
            results['execution_status'] = 'failed'
            results['error'] = str(e)
        
        return results
    
    def _convert_action_plan_to_playbook(self, action_plan):
        """Convert agent's high-level action plan to executable playbook format"""
        playbook = {
            'name': 'Agent-Designed Playbook',
            'steps': []
        }
        
        for phase in action_plan.get('action_sequence', []):
            for action in phase.get('actions', []):
                playbook['steps'].append({
                    ...
                    'reassessment_gate': action.get('reassessment_gate', {})
                })
        
        return playbook
    
    def _display_agent_rationale(self, action_plan):
        """Display agent's reasoning to user/thought trail"""
        rationale = action_plan.get('agent_rationale', {})
        selected_caps = action_plan.get('selected_capabilities', [])
        
        logger.info("="*60)
        logger.info("SPECIALIST AGENT ACTION PLAN")
        logger.info("="*60)
        logger.info(f"Problem Understanding: {rationale.get('problem_understanding', 'N/A')}")
        logger.info(f"\nSelected Capabilities ({len(selected_caps)}):")
        for cap in selected_caps[:5]:  # Show top 5
            logger.info(f"  - {cap.get('capability', 'Unknown')} (relevance: {cap.get('relevance_score', 0):.2f})")
        logger.info(f"\nExpected Outcome: {rationale.get('expected_outcome', 'N/A')}")
        logger.info("="*60)
    
    def _execute_sequential_phase(self, phase, context, results):
        """Execute phase actions sequentially"""
        phase_results = []
        for action in phase.get('actions', []):
            try:
                action_result = self.action_registry.get_action(action.get('tool'))(
                    **action.get('inputs', {}),
                    **context or {}
                )
                phase_results.append({'action': action.get('action'), 'result': action_result})
            except Exception as e:
                logger.error(f"Action {action.get('action')} failed: {e}")
                phase_results.append({'action': action.get('action'), 'error': str(e)})
        
        return {
            'phase': phase.get('phase'),
            'actions': phase_results,
            'status': 'completed'
        }
    
    def _execute_parallel_phase(self, phase, context, results):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(actions)) as executor:
            futures = {
                executor.submit(self._execute_single_action, action, context, results): action 
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    parallel_results.append({'action': action.get('action'), 'result': action_result})
                except Exception as e:
                    logger.error(f"Parallel action {action.get('action')} failed: {e}")
                    parallel_results.append({'action': action.get('action'), 'error': str(e)})
        
        return {
            'phase': phase.get('phase'),
            'actions': parallel_results,
            'status': 'completed',
            'executed_in_parallel': True
        }
    
    def _execute_single_action(self, action, context, results):
        """Execute a single action"""
        tool_name = action.get('tool')
        if not tool_name:
            logger.error(f"Action missing tool name: {action}")
            return {'error': 'No tool specified'}
        
        try:
            # Get the action function from registry
            action_func = self.action_registry.get_action(tool_name)
            
            # Prepare inputs
            inputs = action.get('inputs', {})
            if isinstance(inputs, dict):
                # Merge with context
                inputs = {**inputs, **(context or {})}
            else:
                inputs = context or {}
            ...
            logger.error(f"Failed to execute action {tool_name}: {e}")
            return {'error': str(e), 'action': tool_name}
    
    def execute_workflow_dynamically(self, workflow_file_path: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """Execute a workflow file dynamically"""
        import json
        ...
        try:
            ...
            logger.info(f"Executing workflow: {workflow_data.get('workflow_name', 'Unknown')}")
            
            # Execute workflow tasks
            results = {}
            for task_name, task_def in workflow_data.get('tasks', {}).items():
                action_type = task_def.get('action_type', 'unknown')
                inputs = task_def.get('inputs', {})
                
                try:
                    action_func = self.action_registry.get_action(action_type)
                    task_result = action_func(**inputs, **(context or {}))
            ...
            logger.error(f"Failed to execute workflow {workflow_file_path}: {e}")
            return {'error': str(e), 'workflow': workflow_file_path}
    
    def execute_action_directly(self, action_name: str, inputs: Dict[str, Any] = None, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if context:
                combined_inputs.update(context)
            
            result = action_func(**combined_inputs)
            return {'action': action_name, 'result': result, 'status': 'success'}
            
        except Exception as e:
            logger.error(f"Failed to execute action {action_name}: {e}")
            return {'action': action_name, 'error': str(e), 'status': 'failed'}
    
    def _needs_reassessment(self, phase_result, phase):
        """Check if reassessment is needed"""
        for action_result in phase_result.get('actions', []):
            if 'result' in action_result:
                result = action_result['result']
                iar = result.get('iar_reflection', {}) if isinstance(result, dict) else {}
                confidence = iar.get('confidence', 1.0) if isinstance(iar, dict) else 1.0
